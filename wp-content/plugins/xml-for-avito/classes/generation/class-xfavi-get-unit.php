...
class XFAVI_Get_Unit {
	...
	protected function create_code() {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			for ( $i = 0; $i < $variation_count; $i++ ) {
				...
				$one_variable = xfavi_optionGET( 'xfavi_one_variable', $this->feed_id, 'set_arr' );
				if ( $one_variable == 'on' ) {
					break;
				}

				$stop_flag = false;
				// потенциально заменить фильтр...
				$n = '';
				$special_data_for_flag = '';
				$stop_flag = apply_filters( 'xfavi_after_variable_offer_stop_flag', $stop_flag, $i, $n, $variation_count, $offer->get_id(), $offer, $special_data_for_flag, $this->feed_id );
				if ( $stop_flag == true ) {
					break;
				}
			}
	...
	// ожидается потомок класса XFAVI_Get_Unit_Offer
