...
/**
 * Traits ProductSubType for simple products
 *
...
trait XFAVI_T_Simple_Get_ProductSubType {
	/**
	...
	public function get_product_sub_type( $tag_name = 'ProductSubType', $result_xml = '' ) {
		...
		$tag_value = apply_filters(
			'xfavi_f_simple_tag_value_product_sub_type',
			$tag_value,
			[ 
		...
		if ( ! empty( $tag_value ) ) {
			$tag_name = apply_filters(
				'xfavi_f_simple_tag_name_product_sub_type',
				$tag_name,
				[ 
					'product' => $this->get_product()
				],
		...
		$result_xml = apply_filters(
			'xfavi_f_simple_tag_product_sub_type',
			$result_xml,
			[ 
				'product' => $this->get_product()
			],
