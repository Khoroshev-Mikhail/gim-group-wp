...
trait XFAVI_T_Variable_Get_Image {
	public function get_image($tag_name = 'Image', $result_xml = '') {
		...
		if (($xfavi_skip_products_without_pic === 'on') && ($picture_xml == '')) {				
			$this->add_skip_reason(array('offer_id' => $offer->get_id(), 'reason' => __('Product has no images', 'xfavi'), 'post_id' => $offer->get_id(), 'file' => 'trait-xfavi-t-variable-get-picture.php', 'line' => __LINE__)); return '';
		}
	...
	//	$result_xml = xfavi_replace_domain($result_xml, $this->get_feed_id());
		$result_xml = apply_filters('xfavi_f_variable_tag_picture', $result_xml, array('product' => $product, 'offer' => $offer), $this->get_feed_id());
		return $result_xml;
	}
}
?>