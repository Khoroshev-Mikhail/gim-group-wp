...
final class XFAVI_Plugin_Form_Activate {
	...
	public function __construct( $pref = 'xfavip', $slug = '' ) {
		$this->pref = $pref;
		$this->slug = $slug;
	...
	private function init_hooks() {
		add_action( 'x4avi_activation_forms', [ $this, 'the_form' ] );
		// TODO: Удалить в следующих версиях
		add_action( 'xfavi_before_support_project', [ $this, 'the_form' ] );
	}
							...
							<tr>
								...
								<td class="overalldesc">
									...
									<span class="description">
										...
										); ?>"><?php _e( 'Read more', 'xml-for-avito' ); ?></a>
									</span>
								</td>
							</tr>
							<tr>
								<th scope="row">
									<?php _e( 'Order Email', 'xml-for-avito' ); ?>
								</th>
								<td class="overalldesc">
									...
									<span class="description">
										...
										); ?>"><?php _e( 'Read more', 'xml-for-avito' ); ?></a></span>
								</td>
							</tr>
						</tbody>
					</table>
					<input class="button-primary" name="<?php echo esc_attr( $this->get_submit_name() ); ?>"
						value="<?php _e( 'Update License Data', 'xml-for-avito' ); ?>" type="submit" />
				</form>
			</div>
		</div>
		<?php
	...
	private function save_form() {
		if ( isset( $_REQUEST[ $this->get_submit_name()] ) ) {
			...
			$message = sprintf( '%1$s <a href="javascript:location.reload(true)">%2$s</a>',
				__( 'License data has been updated', 'xml-for-avito' ),
				__( 'Refresh this page', 'xml-for-avito' )
			);
