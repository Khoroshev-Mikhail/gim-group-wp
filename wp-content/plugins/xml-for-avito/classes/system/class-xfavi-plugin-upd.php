...
final class XFAVI_Plugin_Upd {
	const API_URL = 'https://icopydoc.ru/api/v1';
	private $list_plugin_names = [
		'xfavip' => ['name' => 'PRO', 'code' => 'renewlicense20avip']
	];
	...
	public function __construct( $args = [] ) {
		...
		$this->list_plugin_names = apply_filters( 'xfavi_f_list_plugin_names', $this->list_plugin_names, $args );
		do_action_ref_array( 'xfavi_a_plugin_upd', $args );
	...
	public function add_plugin_action_links( $actions, $plugin_file ) {
		...
		switch ( $i ) {
			case "202":
				$message = __( 'License is active', 'xml-for-avito' );
				$color = 'green';
				break;
			case "402":
				$message = __( 'License expired', 'xml-for-avito' );
				$color = '#dc3232';
				break;
			case "412":
				$message = __( 'License data is invalid', 'xml-for-avito' );
				$color = '#dc3232';
				break;
			case "418":
				$message = __(
	...
	public function get_info() {
		...
		switch ( $c ) {
			...
			case "402":
				$message = sprintf(
					'<span style="font-weight: 700;">XML for Avito %1$s:</span> %2$s! %3$s, <a href="https://icopydoc.ru/product/%4$s/?utm_source=%4$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-expired" target="_blank">%5$s</a> (%6$s: <span style="font-weight: 700;">%7$s</span>). %8$s <a href="/wp-admin/admin.php?page=xfaviexport">%9$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expired', 'xml-for-avito' ),
					__( 'Please', 'xml-for-avito' ),
					$this->get_slug(),
					__( 'purchase a new license', 'xml-for-avito' ),
					__( 'to get a discount, use this promo code', 'xml-for-avito' ),
					$this->list_plugin_names[ $this->get_pref()]['code'],
					__( 'If you have already done this', 'xml-for-avito' ),
					__( 'enter the new license information here', 'xml-for-avito' )
				);
				break;
			case "412":
				$message = sprintf( '<span style="font-weight: 700;">XML for Avito %1$s:</span> %2$s! %1$s %3$s. <a href="/wp-admin/admin.php?page=xfaviexport">%4$s</a> %5$s <a href="https://icopydoc.ru/product/%6$s/?utm_source=%6$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-err&utm_term=notice" target="_blank">%7$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License data is invalid', 'xml-for-avito' ),
					__( 'version features do not work and you can not install updates', 'xml-for-avito' ),
					__( 'Enter your license information', 'xml-for-avito' ),
					__( 'or', 'xml-for-avito' ),
					$this->get_slug(),
					__( 'purchase a new one', 'xml-for-avito' )
				);
				break;
			case "418":
				$message = sprintf( '<span style="font-weight: 700;">XML for Avito %1$s:</span> %2$s! <a href="/wp-admin/admin.php?page=xfaviexport">%3$s</a> %4$s <a href="https://icopydoc.ru/product/%5$s/?utm_source=%5$s&utm_medium=organic&utm_campaign=in-plugin&utm_content=license-limit&utm_term=notice" target="_blank">%6$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__(
						'This license cannot be used on this site. The package limit has been exceeded',
						'xml-for-avito'
		...
		if ( $c !== '0' ) {
			...
			if ( $remaining_days > 0 && $remaining_days < 8 ) {
				$message = sprintf(
					'<span style="font-weight: 700;">XML for Avito %1$s:</span> %2$s <span style="font-weight: 700; color: red;">%3$s</span>. %4$s, <a href="https://icopydoc.ru/product/%5$s/?utm_source=link&utm_medium=organic&utm_campaign=in-plugin&utm_content=notice&utm_term=license-remaining" target="_blank">%6$s</a> (%7$s: <span style="font-weight: 700;">%8$s</span>). %9$s <a href="/wp-admin/admin.php?page=xfaviexport">%10$s</a>.',
					$this->list_plugin_names[ $this->get_pref()]['name'],
					__( 'License expires in', 'xml-for-avito' ),
					$this->num_decline( $remaining_days, [ 
						__( 'day', 'xml-for-avito' ),
						_x( 'days', '2 days', 'xml-for-avito' ),
						_x( 'days', '5 days', 'xml-for-avito' )
					]
					),
					__( 'Please', 'xml-for-avito' ),
					$this->get_slug(),
	 ...
	 *
	 * Примеры вызова:
	 * xfavip_num_decline($num, 'книга,книги,книг')
	 * xfavip_num_decline($num, ['книга','книги','книг'])
	 * xfavip_num_decline($num, 'книга', 'книги', 'книг')
	 * xfavip_num_decline($num, 'книга', 'книг')
	 *
	...
	private function get_body_request() {
		$body_request = [ 
			...
			'basic_version' => XFAVI_PLUGIN_VERSION,
			'license_key' => $this->get_license_key(),
			'order_id' => $this->get_order_id(),
			'order_email' => $this->get_order_email(),
			'order_home_url' => home_url( '/' ),
		];
		new XFAVI_Error_Log( $body_request );
		return $body_request;
	}
	...
	private function response_to_api() {
		...
		$api_url = apply_filters( 'xfavi_f_api_url', self::API_URL );
		...
		if ( isset( $options['hook_extra']['plugin'] ) ) {
			if ( $options['hook_extra']['plugin'] === $this->plugin_slug ) {
				...
				$package_url = sprintf(
					...
					XFAVI_PLUGIN_VERSION
				);
				$package_url = apply_filters( 'xfavi_f_package_url', $package_url, $options );
