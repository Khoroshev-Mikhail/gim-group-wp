...
if ( ! class_exists( 'ICPD_Feedback' ) ) {
	final class ICPD_Feedback {
		...
		public function init_hooks() {
			...
			if ( isset( $_REQUEST[ $this->get_submit_name()] ) ) {
				...
				add_action( 'admin_notices', function () {
					...
					$message = __( 'The data has been sent. Thank you', 'xml-for-avito' );
		...
		public function print_view_html_feedback_block() { ?>
			<div class="postbox">
				<h2 class="hndle">
					<?php _e( 'Send data about the work of the plugin', 'xml-for-avito' ); ?>
				</h2>
				<div class="inside">
					<?php
					printf( '<p>%s! %s:</p>',
						__( 'Sending statistics you help make the plugin even better', 'xml-for-avito' ),
						__( 'The following data will be sent', 'xml-for-avito' )
					);
					?>
					<ul class="icpd_ul">
						<?php
						printf( '<li>%s</li><li>%s</li><li>%s %s</li></p>',
							__( 'PHP version information', 'xml-for-avito' ),
							__( 'Multisite mode status', 'xml-for-avito' ),
							__( 'Technical information and plugin logs', 'xml-for-avito' ),
					...
					<form action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>" method="post" enctype="multipart/form-data">
						<?php
						printf( '<p>%s %s?</p>',
							__( 'Did my plugin help you upload your products to the', 'xml-for-avito' ),
							esc_html( $this->get_plugin_name() )
						);
						?>
						<p>
						...
						<p>
							<?php _e( 'Your message', 'xml-for-avito' ); ?>:
						</p>
						<p><textarea rows="6" cols="32" name="<?php echo $this->get_textarea_name(); ?>" placeholder="<?php
						   printf( '%1$s (%2$s). %3$s',
						   	__( 'Enter your text to send me a message', 'xml-for-avito' ),
						   	__( 'You can write me in Russian or English', 'xml-for-avito' ),
						   	__( 'I check my email several times a day', 'xml-for-avito' )
						   ); ?>"></textarea></p>
						...
						<input class="button-primary" type="submit" name="<?php echo $this->get_submit_name(); ?>"
							value="<?php _e( 'Send data', 'xml-for-avito' ); ?>" />
					</form>
				</div>
			</div>
			<?php
		...
		public function send_data() {
			if ( ! empty( $_POST )
				...
				$mail_content .= sanitize_text_field( $additional_info );
